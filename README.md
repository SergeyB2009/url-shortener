...
- **`flushClicksToDb` использует `GETDEL`** — атомарную операцию Redis. Если процесс упадёт после `GETDEL`, но до `UPDATE` в БД, клики за один интервал (≤30 сек) потеряются. Это осознанный компромисс: лучше потерять один интервал, чем задвоить счётчики.
- **`cacheScan` использует `SCAN`**, а не `KEYS` — не блокирует Redis при большом количестве ключей.
- **`getStats` тоже использует `GETDEL`** для буфера, чтобы избежать гонки с `flushClicksToDb`. Если flush и stats запросят буфер одновременно — значение достанется только одному.
